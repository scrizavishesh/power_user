...
const PaymentURl = () => {
  ...
  useEffect(() => {
    const socket = new WebSocket(`wss://auth2.upicollect.com/ws/order_status/${order_id}/?token=50bac6ff4fc2d47fa778c6005d547a526a1d827d`);
  ...
  const handleGooglePay = () => {
    if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
      window.location.href = `intent://pay?pa=${scannerData?.upi_id}&amp;am=${scannerData?.amount}&amp;cu=INR&amp;tn=${scannerData?.receipt_id}#Intent;scheme=upi;package=com.google.android.apps.nbu.paisa.user`;
    } else {
      window.location.href = `https://pay.google.com/gp/v/gpc?amount=${scannerData?.amount}`;
    }
  };

  const handlePhonePay = () => {
    if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
      window.location.href = `intent://pay?pa=${scannerData?.upi_id}&amp;am=${scannerData?.amount}&amp;cu=INR&amp;tn=${scannerData?.receipt_id}#Intent;scheme=upi;package=com.phonepe.app`;
    } else {
  ...
  const handlePaytmPay = () => {
    if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
      window.location.href = `intent://pay?pa=${scannerData?.upi_id}&amp;am=${scannerData?.amount}&amp;cu=INR&amp;tn=${scannerData?.receipt_id}#Intent;scheme=upi;package=net.one97.paytm;end`;
    } else {
  ...
  const handleUPI = () => {
    if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
      window.location.href = `upi://pay?pa=${scannerData?.upi_id}&pn=demo2&tn=${scannerData?.receipt_id}&am=${scannerData?.amount}&cu=INR`;
    } else {
      window.location.href = `https://pay.google.com/gp/v/gpc?amount=${scannerData?.amount}`;
    }
  };
      ...
      <main className="payment-container">
        <div className="payment-card">
          ...
          <footer className="payment-footer">
            <button onClick={handleGooglePay} type="button" class="btn btn-outline">
            ...
            <button onClick={handlePhonePay} type="button" class="btn btn-outline">
            ...
            <button onClick={handlePaytmPay} type="button" class="btn btn-outline">
              ...
              <div className="modal-content">
                <div className="modal-body text-center">
                  <div className="icon-wrapper">
                    <i className="thumb-icon bi bi-hand-thumbs-up-fill"></i>
                  </div>
                  <h4 className="modal-title success-title">🎉 Payment Successful!</h4>
